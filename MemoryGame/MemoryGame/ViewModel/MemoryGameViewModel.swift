...
class MemoryGameViewModel: ObservableObject {
    ...
    init() {
        self.model = MemoryGame(theme: MemoryThemes.randomOption, numberOfPairs: 20) { pairIndex, theme in
    ...
    func newGame() {
        model = MemoryGame(theme: MemoryThemes.randomOption, numberOfPairs: 20) { pairIndex, theme in
            theme.deck[pairIndex]
        }
    }
}
